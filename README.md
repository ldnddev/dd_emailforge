...
That detects OS/arch, downloads the matching GitHub Release (`linux`/`darwin` × `x86_64`/`aarch64`), and installs `$HOME/.local/bin/dd_emailforge`. Theme → `$HOME/.config/ldnddev/` (existing theme is left alone). Pin a version with `VERSION=v0.6.0`. Uninstall: pipe the same script with `bash -s -- uninstall`.
